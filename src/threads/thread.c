...
static struct list ready_list_mlfqs[64];
...
thread_cal_priority (struct thread *t)
{
  ...
  int nice = t->nice * 2;
  int recent_cpu = t->recent_cpu / 65536; // 65536 = 4 * FIXED_COEF

  t->priority = PRI_MAX - recent_cpu - nice;

  // TODO thread yield condition.
